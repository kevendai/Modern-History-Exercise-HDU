...
skip_in_question_num = False
# 自动跳过按钮
auto_skip = True
# 自动跳过时间
auto_skip_time = 0
# 自动跳过第一页
auto_skip_first_page = True
...
# 计算题库大小
if skip_in_question_num:
    question_number = len(df)
else:
    question_number = len(df) - len(skip)
    ...
    if question_type == '单选题' or question_type == '判断题':
        if option == df['正确答案'][i].strip(' '):
            ...
            if str(df['题号'][i]) in right:
                ...
                right[str(df['题号'][i])] = 1
            if (not str(df['题号'][i]) in wrong and df['难易度'][i] == '易') or (right[str(df['题号'][i])] >= 2) or \
                    (not str(df['题号'][i]) in wrong and right[str(df['题号'][i])] >= 1 and df['难易度'][i] == '中'):
        ...
        optionA.config(command=lambda: None)
        optionB.config(command=lambda: None)
        optionC.config(command=lambda: None)
        optionD.config(command=lambda: None)
        if option == df['正确答案'][i].strip(' '):
            # 删除该题
